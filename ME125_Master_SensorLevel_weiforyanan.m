...
%%!!! mannually reformat the .mrk name to '*_ini.mrk' !!!
...
for i = ee+2:length(y)
    ...
    if strcmp(y{i}(end-5:end), '\ReTHM')
        ...
        bad_coil                    = [];
        path_to_MRI_library         = 'C:\Users\mq42604613\Desktop\ARC_DP_pipeline_test\database_for_MEMES\'; % !!! mannually check !!!
...
%         child_MEMES(pathname,elpfile,hspfile,confile,mrkfile,path_to_MRI_library,bad_coil,'yes');  % found here that 
        ...
        cfg.keepchannel             = 'yes';
        cfg.keeptrial               = 'nan';
        data_clean                  = ft_rejectvisual(cfg, alldata);
        ...
        %%2.6 Epoching 
        trigger_types               = [event.value]';
        % deviants
        deviant_trials              = find(ismember(trigger_types,1)); %trigger number 1 is first deviant
        ...
        predeviants_trials               = find(ismember(trigger_types,8)); %trigger number 1 is first deviant
