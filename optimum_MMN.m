...
deviants = [];
deviants = (repmat(2:7,1,50))'; %make deviant sequence 4 deviants * n but 2 types (freq and intensity) are split into up and down
%% Need 2*4 & 5
deviants = [deviants;ones(50,1)*4;ones(50,1)*7]; %additional 4 and 5 which are gap and duration
...
beeps = [standard lofreq_deviant hifreq_deviant gap_deviant lovol_deviant hivol_deviant dur_deviant];
% Fill the audio playback buffer with the audio data, doubled for stereo
% presentation

PsychPortAudio('FillBuffer', pahandle, [beeps; beeps]);

% Start audio playback
%PsychPortAudio('Start', pahandle, repetitions, startCue, waitForDeviceStart);

beep_length = freq*beepLengthSecs;
start_idxs  = 1:beep_length:(6*beep_length+1);
...
volumes     = [ones(1,4)*0.5 vol(1) vol(2) 0.5]; %set the volumes
port_codes  = 33:40; % Use this array to assign port codes
